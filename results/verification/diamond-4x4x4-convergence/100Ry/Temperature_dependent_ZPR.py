...
zprs_lda80 = []
for t in T_lda:
    prefix = "../80Ry/c.welph.save.pdep200.T%d"%(t)
    tmp = read_ZPR(prefix,skipline=4,enable_gw=False)
...
zprs_lda100 = []
for t in T_lda:
    prefix = "../100Ry/c.welph.save.pdep200.T%d"%(t)
    tmp = read_ZPR(prefix,skipline=4,enable_gw=False)
    zprs_lda100.append(tmp[0])
zprs_lda100 = np.array(zprs_lda100) - zprs_lda100[0]
...
plt.plot(T_lda,zprs_lda100,"o-",label="This work, LDA (Antonius lattice), 100 Ry")
