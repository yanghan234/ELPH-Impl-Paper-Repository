...
for t in T_lda:
    ...
    tmp = read_ZPR(prefix,skipline=4,enable_gw=True)
    zprs_lda.append(tmp[0])
    zprs_lda_gw.append(tmp[1])
zprs_lda = np.array(zprs_lda) - zprs_lda[0]
zprs_lda_gw = np.array(zprs_lda_gw) - zprs_lda_gw[0]

zprs_lda_relax = []
zprs_lda_gw_relax = []
for t in T_lda:
    prefix = "./60Ry-LDA-myrelax/c.welph.save.pdep200.T%d"%(t)
    tmp = read_ZPR(prefix,skipline=4,enable_gw=True)
    zprs_lda_relax.append(tmp[0])
    zprs_lda_gw_relax.append(tmp[1])
zprs_lda_relax = np.array(zprs_lda_relax) - zprs_lda_relax[0]
zprs_lda_gw_relax = np.array(zprs_lda_gw_relax) - zprs_lda_gw_relax[0]
...
for t in T_lda:
    ...
    tmp = read_ZPR(prefix,skipline=4,enable_gw=True)
    zprs_pbe.append(tmp[0])
    zprs_pbe_gw.append(tmp[1])
zprs_pbe = np.array(zprs_pbe) - zprs_pbe[0]
zprs_pbe_gw = np.array(zprs_pbe_gw) - zprs_pbe_gw[0]
...
for t in T_lda:
    ...
    tmp = read_ZPR(prefix,skipline=4,enable_gw=True)
    zprs_pbe_kr.append(tmp[0])
    zprs_pbe_gw_kr.append(tmp[1])
print(zprs_pbe_kr)
zprs_pbe_kr = np.array(zprs_pbe_kr) - zprs_pbe_kr[0]
zprs_pbe_gw_kr = np.array(zprs_pbe_gw_kr) - zprs_pbe_gw_kr[0]
...
for t in T_lda:
    ...
    tmp = read_ZPR(prefix,skipline=4,enable_gw=True)
    zprs_pbe_relax.append(tmp[0])
    zprs_pbe_gw_relax.append(tmp[1])
zprs_pbe_relax = np.array(zprs_pbe_relax) - zprs_pbe_relax[0]
zprs_pbe_gw_relax = np.array(zprs_pbe_gw_relax) - zprs_pbe_gw_relax[0]

T_blyp = [0,100,200,300,400,500,600,700]
zprs_blyp_relax = []
zprs_blyp_gw_relax = []
for t in T_lda:
    prefix = "./60Ry-BLYP-myrelax/c.welph.save.pdep200.T%d"%(t)
    tmp = read_ZPR(prefix,skipline=4,enable_gw=True)
    zprs_blyp_relax.append(tmp[0])
    zprs_blyp_gw_relax.append(tmp[1])
zprs_blyp_relax = np.array(zprs_blyp_relax) - zprs_blyp_relax[0]
zprs_blyp_gw_relax = np.array(zprs_blyp_gw_relax) - zprs_blyp_gw_relax[0]



T_pbe_te = [0,100,250,500,750]
zprs_pbe_te = []
zprs_pbe_gw_te = []
for t in T_pbe_te:
    prefix = "./60Ry-PBE-Arpan/%dK/c.welph.save/"%(t)
    tmp = read_ZPR(prefix,skipline=4,enable_gw=True)
    zprs_pbe_te.append(tmp[0])
    zprs_pbe_gw_te.append(tmp[1])

print("zprs_pbe_te")
print(zprs_pbe_te)
zprs_pbe_te = np.array(zprs_pbe_te) - zprs_pbe_te[0]
print("zprs_pbe_gw_te")
print(zprs_pbe_gw_te)
zprs_pbe_gw_te = np.array(zprs_pbe_gw_te) - zprs_pbe_gw_te[0]
...
plt.plot(antonius_dfpt["T"],antonius_dfpt["zpr"],"s-",label="Antonius et al., LDA")
plt.plot(monserrat_dft["T"],monserrat_dft["zpr"],"d-",label="Monserrat, LDA")
plt.plot(T_lda,zprs_lda,"o-",label="This work, LDA (Antonius lattice)")
...
plt.plot(T_blyp,zprs_blyp_relax,"o-",label="This work, BLYP (relaxed lattice)")
...
plt.plot(antonius_dfpt_gw["T"],antonius_dfpt_gw["zpr"],"s--",label="Antonius et al., LDA+GW")
plt.plot(monserrat_gw["T"],monserrat_gw["zpr"],"d--",label="Monserrat, LDA+GW")
plt.plot(T_lda,zprs_lda_gw,"o--",label="This work, LDA+GW (Antonius lattice)")
plt.plot(T_lda,zprs_lda_gw_relax,"o--",label="This work, LDA+GW (relaxed lattice)")
plt.plot(T_pbe,zprs_pbe_gw_relax,"o--",label="This work, PBE+GW (relaxed lattice)")
plt.plot(T_pbe,zprs_blyp_gw_relax,"o--",label="This work, BLYP+GW (relaxed lattice)")
# plt.plot(T_pbe,zprs_pbe_gw_kr,"o--",label="This work, PBE+GW (Kresse lattice)")
# plt.plot(T_pbe,zprs_pbe_gw,"o--",label="This work, PBE+GW (Arpan lattice@100K)")
# plt.plot(T_pbe_te,zprs_pbe_gw_te,"o--",label="This work, PBE+GW+TE (Arpan lattice)")
plt.plot(gapA1["T"],gapA1["gap"],'^',c='k',label="Type IIa method 1")
plt.plot(gapA2["T"],gapA2["gap"],'v',c='k',label="Type IIa method 2")
plt.legend(fontsize=11,loc="lower left")
plt.xlabel("Temperature [Kelvin]",fontsize=13)
plt.ylabel("Direct gap renormalization [meV]",fontsize=13)
plt.show()
fig.savefig("Diamond_temperature_dependence_GW.png",dpi=200,bbox_inches="tight")
