...
prefix = "./60Ry-PBE-Arpan"
# prefix = "./60Ry"
zprs_adi = []
zprs_nad = []
Ts = range(0,701,100)
for T in Ts:
    folder = os.path.join(prefix,f"c.welph.save.pdep200.T{T:d}")
...
print("T,zpr_adi,zpr_nad")
for T, zpr_adi, zpr_nad in zip(Ts,zprs_adi,zprs_nad):
    print(f"{T:04d},{zpr_adi:.04f},{zpr_nad:.04f}")

zprs_adi = np.array(zprs_adi)
zprs_nad = np.array(zprs_nad)

fig = plt.figure(figsize=(8,4.5))
plt.plot(Ts,zprs_adi-zprs_adi[0],"o-",label="Adiabatic")
plt.plot(Ts,zprs_nad-zprs_nad[0],"^-",label="Nonadiabatic")
plt.xlabel("Temperature [Kelvin]")
plt.ylabel("ZPR [meV]")
plt.legend()
plt.show()
fig.savefig("compare_adi_nad_indirect_gap.png",bbox_inches="tight",dpi=200)
