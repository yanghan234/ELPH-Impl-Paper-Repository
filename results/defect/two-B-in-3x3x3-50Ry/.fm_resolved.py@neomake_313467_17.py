...
defect_fm_adi = fm_adi[fm_adi["ibn"]==108]["fm"].values
gap_fm_adi = fm_adi[fm_adi["ibn"]==109]["fm"].values - \
            fm_adi[fm_adi["ibn"]==107]["fm"].values

fm_nad = pd.read_csv("./b.welph.save.pdep800/reFMselfen_DFT_nad_resolved.tab",
            names=["iks","ibn","iq","imode","en","fm"],skiprows=1,
            sep="\\s+")
defect_fm_nad = fm_nad[fm_nad["ibn"]==108]["fm"].values
gap_fm_nad = fm_nad[fm_nad["ibn"]==109]["fm"].values - \
            fm_nad[fm_nad["ibn"]==107]["fm"].values
...
for i in range(len(defect_fm_adi)):
    ...
    gap_fm_diff.append((i+1),gap_fm_adi[i],gap_fm_nad[i],gap_fm_nad[i]-gap_fm_aid[i])
...
# plt.plot(range(fm_adi[fm_adi["ibn"]==108].shape[0]),fm_adi[fm_adi["ibn"]==108]["fm"],"o",label="adiabatic")
# plt.plot(range(fm_nad[fm_nad["ibn"]==108].shape[0]),fm_nad[fm_nad["ibn"]==108]["fm"],"o",label="non-adiabatic")
fig = plt.figure(figsize=(8,4.5))
plt.plot(range(fm_nad[fm_nad["ibn"]==108].shape[0]),fm_nad[fm_nad["ibn"]==108]["fm"]-fm_adi[fm_adi["ibn"]==108]["fm"],"o",label="Nonadiabatic-Adiabatic")
plt.grid()
