...
plt.plot(range(1,1+len(defect_fm_diff)),defect_fm_nad-defect_fm_adi,"o",label="Defect state ZPR")
plt.plot(range(1,1+len(gap_fm_diff)),gap_fm_nad-gap_fm_adi,"o",label="Gap ZPR")
plt.grid()
plt.xlabel("Index of phonon mode")
plt.ylabel("Difference in ZPR [meV]")
plt.legend()
plt.show()
fig.savefig("difference_in_zpr_boron4.png",dpi=300,bbox_inches="tight")
